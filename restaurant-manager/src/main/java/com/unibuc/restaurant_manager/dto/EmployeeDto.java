...
public class EmployeeDto extends UserDto {
    ...
    @NotNull(groups = OnCreate.class, message = "birthDate is required and cannot be blank")
    @Past(message = "birthDate date must be in the past")
    @JsonFormat(pattern = "dd-MM-yyyy")
    private LocalDate birthDate;

    @NotBlank(groups = OnCreate.class, message = "CNP is required and cannot be blank")
    @Pattern(regexp = "\\d{13}", message = "CNP must be exactly 13 digits")
    private String CNP;

    @NotBlank(groups = OnCreate.class, message = "IDSeries is required and cannot be blank")
    @Pattern(regexp = "[A-Z]{2}", message = "IDSeries must contain exactly 2 uppercase letters")
    private String IDSeries;

    @NotBlank(groups = OnCreate.class, message = "IDNumber is required and cannot be blank")
